...
dom('quicksort',e('arr',_),[],int([-2147483648..2147483647])).
% add new array domain e.g.:
%  dom('yourFunName',e('yourArray',_),[],int([min..max])).

create_input_vals('quicksort',Ins):-
  create_input_val('n',int([-2147483648..2147483647]),Ins),
  create_input_val(dim('arr'),int([3..3]),Ins),
...
unquantif_preconds('quicksort',[]).

quantif_preconds('quicksort',[]).
