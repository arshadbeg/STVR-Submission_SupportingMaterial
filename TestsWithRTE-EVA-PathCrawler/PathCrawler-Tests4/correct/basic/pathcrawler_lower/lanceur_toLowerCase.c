...
int main(int pathcrawler_argc, char *pathcrawler_argv[])
{
  ...
  {/* start global loop, one iteration for each test case */
    ...
    {/* start dimension loop, reading the dimensions for current test case */
      ...
      if(pathcrawler_strcmp(pathcrawler_inds.name_base,"str")==0 &&(pathcrawler_inds.num_inds == 0) &&(pathcrawler_inds.num_fields == 0) ){
        pathcrawler_scan(PATHCRAWLER_INT,&pathcrawler_dim);
        /* debug */ //pathcrawler_errormsg("pathcrawler_dim(str) = %d\n",pathcrawler_dim);
        if(pathcrawler_dim<0) {
          pathcrawler_errormsg("pathcrawler_dim(str) < 0\n");
          exit(0);
        } else { 
          if(pathcrawler_dim==0) str = pathcrawler_null();
          else { /* pathcrawler_dim > 0 */
            str = pathcrawler_array_alloc(pathcrawler_dim, sizeof(char));
            if(str== pathcrawler_null())
              pathcrawler_error("Warning : insufficient memory in a malloc call. Aborting\n");
            pathcrawler_addresses[pathcrawler_n_address] = str;
            ...
            Pre_str = pathcrawler_array_alloc(pathcrawler_dim, sizeof(char));
            if(Pre_str== pathcrawler_null())
              pathcrawler_error("Warning : insufficient memory in a malloc call. Aborting\n");
            pathcrawler_addresses[pathcrawler_n_address] = Pre_str;
            /* debug malloc */ // printf("malloc : pathcrawler_addresses[%d]=%d\n",pathcrawler_n_address,pathcrawler_addresses[pathcrawler_n_address]); fflush(stdout);
            pathcrawler_n_address++;
          }
        }
    ...
    {/* start values loop, reading variable values for current test case */ 
      ...
      if(pathcrawler_strcmp(pathcrawler_inds.name_base,"str")==0 &&(pathcrawler_inds.num_inds == 1) &&(pathcrawler_inds.num_fields == 0) ){
        pathcrawler_scan(PATHCRAWLER_CHAR, &pathcrawler_d);
        if(pathcrawler_d<(-127-1) || pathcrawler_d>127)
          { pathcrawler_errormsg( "str[%d] out of range [%d,%d]\n",pathcrawler_inds.tab_inds[0],(-127-1),127);
            exit(0); }
        str[pathcrawler_inds.tab_inds[0]] = pathcrawler_d;
        Pre_str[pathcrawler_inds.tab_inds[0]] = pathcrawler_d;
        /* debug */ //pathcrawler_errormsg("str[%d]=%d\n",pathcrawler_inds.tab_inds[0],pathcrawler_d);
         continue;
      }
...
void pathcrawler_call_oracle(void){
      oracle_toLowerCase(Pre_str, str);
}
