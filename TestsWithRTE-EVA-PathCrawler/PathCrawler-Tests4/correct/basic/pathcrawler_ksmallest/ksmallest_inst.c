...
int partition(int *arr, int low, int high)
{
  ...
  while (1) {
    if (j < high) {
      pathcrawler_trace_full("+278","l.5, file ksmallest.c");
      goto lin_for_body_2;
    }
    else {
      pathcrawler_trace_full("-278","l.5, file ksmallest.c");
    ...
    lin_for_body_2:
    if (*(arr + j) <= pivot) {
      pathcrawler_trace_full("+283","l.6, file ksmallest.c");
      lin_ite_body_7:
      {
        i ++;
        int temp = *(arr + i);
...
int quickSelect(int *arr, int low, int high, int k)
{
  ...
  int __tmp_lin_0;
  if (low <= high) {
    pathcrawler_trace_full("+301","l.20, file ksmallest.c");
    lin_ite_body_22:
    {
      __tmp_lin_0 = partition(arr,low,high);
      int pi = __tmp_lin_0;
      if (pi == k - 1) {
        pathcrawler_trace_full("+307","l.23, file ksmallest.c");
        lin_ite_body_12: {
                           __retres = *(arr + pi);
                           goto return_label;
                         }
      ...
      if (pi > k - 1) {
        pathcrawler_trace_full("+315","l.24, file ksmallest.c");
        lin_ite_body_17:
        {
