...
dom('kthSmallest',e('arr',_),[],int([-2147483648..2147483647])).
% add new array domain e.g.:
%  dom('yourFunName',e('yourArray',_),[],int([min..max])).

create_input_vals('kthSmallest',Ins):-
...
unquantif_preconds('kthSmallest',[]).

quantif_preconds('kthSmallest',[]).

strategy('kthSmallest',[]).
