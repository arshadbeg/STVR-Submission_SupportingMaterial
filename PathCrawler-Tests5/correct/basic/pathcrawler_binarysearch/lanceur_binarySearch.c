...
int main(int pathcrawler_argc, char *pathcrawler_argv[])
{
  ...
  {/* start global loop, one iteration for each test case */
    ...
    {/* start values loop, reading variable values for current test case */ 
      ...
      if(pathcrawler_strcmp(pathcrawler_name,"target")==0 ){
        pathcrawler_scan(PATHCRAWLER_INT, &pathcrawler_d);
        if(pathcrawler_d<(-2147483647-1) || pathcrawler_d>2147483647)
          { pathcrawler_errormsg( "target out of range [%d,%d]\n",(-2147483647-1),2147483647);
            exit(0); }
        target = pathcrawler_d;
        Pre_target = pathcrawler_d;
        /* debug */ //pathcrawler_errormsg("target=%d\n",pathcrawler_d);
         continue;
      }

      if(pathcrawler_strcmp(pathcrawler_name,"size")==0 ){
        pathcrawler_scan(PATHCRAWLER_INT, &pathcrawler_d);
        if(pathcrawler_d<(-2147483647-1) || pathcrawler_d>2147483647)
          { pathcrawler_errormsg( "size out of range [%d,%d]\n",(-2147483647-1),2147483647);
            exit(0); }
        size = pathcrawler_d;
        Pre_size = pathcrawler_d;
        /* debug */ //pathcrawler_errormsg("size=%d\n",pathcrawler_d);
         continue;
      }
...
void pathcrawler_call_fonction(void){
      #define main pathcrawler___main
      pathcrawler__retres__binarySearch = binarySearch(arr, size, target);
#undef main
}

void pathcrawler_call_oracle(void){
      oracle_binarySearch(Pre_arr, arr, size, target, pathcrawler__retres__binarySearch);
}
