...
int main(int pathcrawler_argc, char *pathcrawler_argv[])
{
  ...
  {/* start global loop, one iteration for each test case */
    ...
    {/* start dimension loop, reading the dimensions for current test case */
      ...
      if(pathcrawler_strcmp(pathcrawler_inds.name_base,"s")==0 &&(pathcrawler_inds.num_inds == 0) &&(pathcrawler_inds.num_fields == 0) ){
        pathcrawler_scan(PATHCRAWLER_INT,&pathcrawler_dim);
        /* debug */ //pathcrawler_errormsg("pathcrawler_dim(s) = %d\n",pathcrawler_dim);
        if(pathcrawler_dim<0) {
          pathcrawler_errormsg("pathcrawler_dim(s) < 0\n");
          exit(0);
        } else { 
          if(pathcrawler_dim==0) s = pathcrawler_null();
          else { /* pathcrawler_dim > 0 */
            s = pathcrawler_array_alloc(pathcrawler_dim, sizeof(struct Stack));
            if(s== pathcrawler_null())
              pathcrawler_error("Warning : insufficient memory in a malloc call. Aborting\n");
            pathcrawler_addresses[pathcrawler_n_address] = s;
            ...
            Pre_s = pathcrawler_array_alloc(pathcrawler_dim, sizeof(struct Stack));
            if(Pre_s== pathcrawler_null())
              pathcrawler_error("Warning : insufficient memory in a malloc call. Aborting\n");
            pathcrawler_addresses[pathcrawler_n_address] = Pre_s;
            /* debug malloc */ // printf("malloc : pathcrawler_addresses[%d]=%d\n",pathcrawler_n_address,pathcrawler_addresses[pathcrawler_n_address]); fflush(stdout);
            pathcrawler_n_address++;
          }
        }
    ...
    {/* start values loop, reading variable values for current test case */ 
      ...
      if(pathcrawler_strcmp(pathcrawler_inds.name_base,"s")==0 &&(pathcrawler_inds.num_inds == 2) &&(pathcrawler_inds.num_fields == 1) && (pathcrawler_strcmp("arr",pathcrawler_inds.fields[0])==0)){
        pathcrawler_scan(PATHCRAWLER_INT, &pathcrawler_d);
        if(pathcrawler_d<(-2147483647-1) || pathcrawler_d>2147483647)
          { pathcrawler_errormsg( "s[%d].arr[%d] out of range [%d,%d]\n",pathcrawler_inds.tab_inds[0],pathcrawler_inds.tab_inds[1],(-2147483647-1),2147483647);
            exit(0); }
        s[pathcrawler_inds.tab_inds[0]].arr[pathcrawler_inds.tab_inds[1]] = pathcrawler_d;
        Pre_s[pathcrawler_inds.tab_inds[0]].arr[pathcrawler_inds.tab_inds[1]] = pathcrawler_d;
        /* debug */ //pathcrawler_errormsg("s[%d].arr[%d]=%d\n",pathcrawler_inds.tab_inds[0],pathcrawler_inds.tab_inds[1],pathcrawler_d);
         continue;
      }

      if(pathcrawler_strcmp(pathcrawler_inds.name_base,"s")==0 &&(pathcrawler_inds.num_inds == 1) &&(pathcrawler_inds.num_fields == 1) && (pathcrawler_strcmp("top",pathcrawler_inds.fields[0])==0)){
        pathcrawler_scan(PATHCRAWLER_INT, &pathcrawler_d);
        if(pathcrawler_d<(-2147483647-1) || pathcrawler_d>2147483647)
          { pathcrawler_errormsg( "s[%d].top out of range [%d,%d]\n",pathcrawler_inds.tab_inds[0],(-2147483647-1),2147483647);
            exit(0); }
        s[pathcrawler_inds.tab_inds[0]].top = pathcrawler_d;
        Pre_s[pathcrawler_inds.tab_inds[0]].top = pathcrawler_d;
        /* debug */ //pathcrawler_errormsg("s[%d].top=%d\n",pathcrawler_inds.tab_inds[0],pathcrawler_d);
         continue;
      }
