...
int main(int pathcrawler_argc, char *pathcrawler_argv[])
{
  ...
  {/* start global loop, one iteration for each test case */
    ...
    {/* start dimension loop, reading the dimensions for current test case */
      ...
      if(pathcrawler_strcmp(pathcrawler_inds.name_base,"head")==0 &&(pathcrawler_inds.num_inds == 0) &&(pathcrawler_inds.num_fields == 0) ){
        pathcrawler_scan(PATHCRAWLER_INT,&pathcrawler_dim);
        /* debug */ //pathcrawler_errormsg("pathcrawler_dim(head) = %d\n",pathcrawler_dim);
        if(pathcrawler_dim<0) {
          pathcrawler_errormsg("pathcrawler_dim(head) < 0\n");
          exit(0);
        } else { 
          if(pathcrawler_dim==0) head = pathcrawler_null();
          else { /* pathcrawler_dim > 0 */
            head = pathcrawler_array_alloc(pathcrawler_dim, sizeof(struct Node));
            if(head== pathcrawler_null())
              pathcrawler_error("Warning : insufficient memory in a malloc call. Aborting\n");
            pathcrawler_addresses[pathcrawler_n_address] = head;
            ...
            Pre_head = pathcrawler_array_alloc(pathcrawler_dim, sizeof(struct Node));
            if(Pre_head== pathcrawler_null())
              pathcrawler_error("Warning : insufficient memory in a malloc call. Aborting\n");
            pathcrawler_addresses[pathcrawler_n_address] = Pre_head;
            /* debug malloc */ // printf("malloc : pathcrawler_addresses[%d]=%d\n",pathcrawler_n_address,pathcrawler_addresses[pathcrawler_n_address]); fflush(stdout);
            pathcrawler_n_address++;
          }
        }
      ...
      if(pathcrawler_strcmp(pathcrawler_inds.name_base,"head")==0 &&(pathcrawler_inds.num_inds == 1) &&(pathcrawler_inds.num_fields == 1) && (pathcrawler_strcmp("next",pathcrawler_inds.fields[0])==0)){
        pathcrawler_scan(PATHCRAWLER_INT,&pathcrawler_dim);
        /* debug */ //pathcrawler_errormsg("pathcrawler_dim(head[%d].next) = %d\n",pathcrawler_inds.tab_inds[0],pathcrawler_dim);
        if(pathcrawler_dim<0) {
          pathcrawler_errormsg("pathcrawler_dim(head[%d].next) < 0\n",pathcrawler_inds.tab_inds[0]);
          exit(0);
        } else { 
          if(pathcrawler_dim==0) head[pathcrawler_inds.tab_inds[0]].next = pathcrawler_null();
          else { /* pathcrawler_dim > 0 */
            head[pathcrawler_inds.tab_inds[0]].next = pathcrawler_array_alloc(pathcrawler_dim, sizeof(struct Node));
            if(head[pathcrawler_inds.tab_inds[0]].next== pathcrawler_null())
              pathcrawler_error("Warning : insufficient memory in a malloc call. Aborting\n");
            pathcrawler_addresses[pathcrawler_n_address] = head[pathcrawler_inds.tab_inds[0]].next;
            /* debug malloc */ // printf("malloc : pathcrawler_addresses[%d]=%d\n",pathcrawler_n_address,pathcrawler_addresses[pathcrawler_n_address]); fflush(stdout);
            pathcrawler_n_address++;
            Pre_head[pathcrawler_inds.tab_inds[0]].next = pathcrawler_array_alloc(pathcrawler_dim, sizeof(struct Node));
            if(Pre_head[pathcrawler_inds.tab_inds[0]].next== pathcrawler_null())
              pathcrawler_error("Warning : insufficient memory in a malloc call. Aborting\n");
            pathcrawler_addresses[pathcrawler_n_address] = Pre_head[pathcrawler_inds.tab_inds[0]].next;
            /* debug malloc */ // printf("malloc : pathcrawler_addresses[%d]=%d\n",pathcrawler_n_address,pathcrawler_addresses[pathcrawler_n_address]); fflush(stdout);
            pathcrawler_n_address++;
          }
        }
    ...
    {/* start values loop, reading variable values for current test case */ 
      ...
      if(pathcrawler_strcmp(pathcrawler_inds.name_base,"head")==0 &&(pathcrawler_inds.num_inds == 1) &&(pathcrawler_inds.num_fields == 1) && (pathcrawler_strcmp("data",pathcrawler_inds.fields[0])==0)){
        pathcrawler_scan(PATHCRAWLER_INT, &pathcrawler_d);
        if(pathcrawler_d<(-2147483647-1) || pathcrawler_d>2147483647)
          { pathcrawler_errormsg( "head[%d].data out of range [%d,%d]\n",pathcrawler_inds.tab_inds[0],(-2147483647-1),2147483647);
            exit(0); }
        head[pathcrawler_inds.tab_inds[0]].data = pathcrawler_d;
        Pre_head[pathcrawler_inds.tab_inds[0]].data = pathcrawler_d;
        /* debug */ //pathcrawler_errormsg("head[%d].data=%d\n",pathcrawler_inds.tab_inds[0],pathcrawler_d);
         continue;
      }
...
void pathcrawler_call_fonction(void){
      #define main pathcrawler___main
      pathcrawler__retres__insertAtHead = insertAtHead(head, value);
#undef main
}

void pathcrawler_call_oracle(void){
      oracle_insertAtHead(Pre_head, head, value, pathcrawler__retres__insertAtHead);
}
