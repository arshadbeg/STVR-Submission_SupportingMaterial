...
int main(int pathcrawler_argc, char *pathcrawler_argv[])
{
  ...
  {/* start global loop, one iteration for each test case */
    ...
    {/* start values loop, reading variable values for current test case */ 
      ...
      if(pathcrawler_strcmp(pathcrawler_name,"b")==0 ){
        pathcrawler_scan(PATHCRAWLER_INT, &pathcrawler_d);
        if(pathcrawler_d<(-2147483647-1) || pathcrawler_d>2147483647)
          { pathcrawler_errormsg( "b out of range [%d,%d]\n",(-2147483647-1),2147483647);
            exit(0); }
        b = pathcrawler_d;
        Pre_b = pathcrawler_d;
        /* debug */ //pathcrawler_errormsg("b=%d\n",pathcrawler_d);
         continue;
      }

      if(pathcrawler_strcmp(pathcrawler_name,"a")==0 ){
        pathcrawler_scan(PATHCRAWLER_INT, &pathcrawler_d);
        if(pathcrawler_d<(-2147483647-1) || pathcrawler_d>2147483647)
          { pathcrawler_errormsg( "a out of range [%d,%d]\n",(-2147483647-1),2147483647);
            exit(0); }
        a = pathcrawler_d;
        Pre_a = pathcrawler_d;
        /* debug */ //pathcrawler_errormsg("a=%d\n",pathcrawler_d);
         continue;
      }
...
void pathcrawler_call_fonction(void){
      #define main pathcrawler___main
      pathcrawler__retres__gcd = gcd(a, b);
#undef main
}

void pathcrawler_call_oracle(void){
      oracle_gcd(a, b, pathcrawler__retres__gcd);
}
