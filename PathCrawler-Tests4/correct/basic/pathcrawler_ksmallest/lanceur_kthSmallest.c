    ...
    {/* start values loop, reading variable values for current test case */ 
      ...
      if(pathcrawler_strcmp(pathcrawler_name,"k")==0 ){
        ...
        if(pathcrawler_d<(-2147483647-1) || pathcrawler_d>2147483647)
          { pathcrawler_errormsg( "k out of range [%d,%d]\n",(-2147483647-1),2147483647);
            exit(0); }
        k = pathcrawler_d;
        Pre_k = pathcrawler_d;
        /* debug */ //pathcrawler_errormsg("k=%d\n",pathcrawler_d);
         continue;
      }
...
void pathcrawler_call_fonction(void){
      #define main pathcrawler___main
      pathcrawler__retres__kthSmallest = kthSmallest(arr, size, k);
#undef main
}

void pathcrawler_call_oracle(void){
      oracle_kthSmallest(Pre_arr, arr, size, k, pathcrawler__retres__kthSmallest);
}
